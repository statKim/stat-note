...
\begin{itemize}
	...
	\begin{proof}[\underline{\textbf{Solution}}] $\newline$
            	(i) For arbitrary set $A \in \calB$, $\mu(A) \ge 0$ and $\nu(A) \ge 0$. $\Rightarrow \eta(A) = \mu(A) + \nu(A) \ge 0.$ \\
            	$\therefore \eta: \calB \rightarrow [0, \infty].$
            	
            	(ii) For disjoint set $B_1, B_2, \dots \subset \calB$, then
            	\begin{align*}
	...
	\begin{proof}[\underline{\textbf{Solution}}] $\newline$
		(i) Let $f = \sum_{i=1}^n a_i 1_{A_i}$, the non-negative simple function. Then,
		\begin{align*}
			\int f d\eta &= \int \sum_{i=1}^n a_i 1_{A_i} d\eta = \sum a_i\eta(A_i) \\
					 &= \sum a_i \left\{ \mu(A_i) + \nu(A_i) \right\}  = \int f d\mu + \int f d\nu.
		\end{align*}

		(ii) For general case, let $f_n$ is the sequence of non-negative simple functions increasing to $f$. (i.e. $f_1 \le f_2 \le \dots \le f$)
		\begin{align*}
...
$$ P(X=a) = F_X(a) - F_X(a-). $$
\begin{proof}[\underline{\textbf{Solution}}] $\newline$
(i) Let $B_n = \{ X \le a-1/n \}$ and $\union_{n=1}^\infty B_n = \{ X < a \}$.
By continuity of measure,
$$ P(B) = P(X < a) = \limn P(X \le a-1/n) = \limn F_X(a-1/n) = F_X(a-). $$
(ii) Since $\{ X<a \}$ and $\{X=a\}$ are disjoint with union $\{X \le a\}$,
