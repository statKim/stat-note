...
A collection of $\calA$ of subsets of a set $\calX$ is a {\color{blue} $\sigma$-algebra (or $\sigma$-field)} if
\begin{enumerate}
...
A function $\mu$ on a $\sigma$-algebra $\calA$ of $\calX$ is a {\color{blue} measure} if
\begin{enumerate}
...
\begin{enumerate}	
	\item A measure $\mu$ is {\color{blue} finite} if $\mu(\calX) < \infty$.
	\item A measure $\mu$ is {\color{blue} $\sigma$-finite} if $\exists A_1,A_2,\dots \in \calA$ with $\mu(A_i) < \infty ~ \forall i = 1, 2, \dots$ and $\union_{i=1}^\infty A_i = \calX$.
\end{enumerate}
\end{definition}
...
\begin{definition}[Measurable]
    If $(\calX, \calA)$ is a measurable space and $f$ is a real-valued function on $\calX$, then $f$ is {\color{blue} measurable} if
    $$ f^{-1}(B) \eqdef \{ x \in \calX : f(x) \in B \} \in \calA, $$
    for every Borel set $B$.
\end{definition}

\begin{definition}[Indicator function]
    The {\color{blue} indicator function} $1_A$ of a set $A$ is defined as
    $$ 
...
A function $f$ is {\color{blue} simple} if
...
% \end{enumerate}
A measurable function $f$ is {\color{blue} integrable} if $\int |f| d\mu < \infty$.
\end{definition}
...
If $\calE$ is a sample space,  $\calB$ is a $\sigma$-algebra of subsets of $\calX$, and $P$ is a probability measure, then the triple $(\calX, \calA, P)$ is called a  {\color{blue} probability space}.
Also $B \in \calB$ are called events, points $e \in \calE$ are called outcomes, and $P(B)$ is called the probability of B.
\end{definition}

\begin{definition}[Random variable]
A measurable function $X: \calE \rightarrow \bbR$ is called a {\color{blue} random variable}.
\end{definition}

\begin{definition}[Distribution]
The probability measure $P_X$ is defined by
$$ P_X(A) = P\left(\{ e \in \calE : X(e) \in A \}\right) \overset{\mathrm{def}}{=} P(X \in A), $$ 
for Borel sets $A$ is called {\color{blue} distribution} of $X$, and if we let $P_X := Q$, then it is denoted as $X \sim Q$, means that a random variable $X$ has distribution $Q$.
\end{definition}

\begin{definition}[Cumulative distribution function]
The {\color{blue} cumulative distribution function} of $X$ is defined by 
...
For a measure $\mu$ on the measurable space $(\calX, \calA)$, a set $N$ is called {\color{blue} null} with respect to $\mu$ if 
$$ \mu(N) = 0. $$
\end{definition}

\begin{definition}[Almost everywhere]
If a statement holds for $x \in \calX-N$ with $N$ null, the statement is said to hold {\color{blue} almost everywhere} (a.e.) or a.e. $\mu$.
\end{definition}
...
Let P and $\mu$ be measures on a $\sigma$-field $\calA$ of $\calX$. Then P is called {\color{blue} absolutely continuous} with respect to $\mu$, written $P \ll \mu$, if $P(A) = 0$ whenever $\mu(A) = 0$.
\end{definition}
...
\begin{remark}
The function $f$ in Theorem \ref{thm:radon} is called Radon-Nikodym deravitive of $P$ with respect to $\mu$, or the {\em density} of $P$ with respect to $\mu$, denoted 
$$ f = \frac{dP}{d\mu}. $$
\end{remark}

\begin{remark}
If $X \sim P_X$ and $P_X$ is absolutely continuous with respect to $\mu$ with density $p = dP_X / d\mu$, it is called that {\em X has density p with respect to $\mu$}.
\end{remark}
...
If $X$ is a random variable on a probability space $(\calE, \calB, P)$, then the {\color{blue} expectation or expected value} of $X$ is defined as 
$$EX = \int X dP.$$
If $X \sim P_X$, then
$$ EX = \int x dP_X(x). $$
Also, if $Y= f(X)$, then
...
The {\color{blue} variance} of a random variable $X$ with finite expectation is defined as
...
The {\color{blue} covariance} between two random variables $X$ and $Y$ with finite expectation is defined as
$$ \Cov(X,Y) = E(X-EX)(Y-EY) = EXY - (EX)(EY). $$
\end{definition}

\begin{definition}[Correlation]
The {\color{blue} correlation} between two random variables $X$ and $Y$ with finite expectation is defined as
...
is called a {\color{blue} random vector}.
\end{definition}
...
\begin{definition}[Random matrix]
	A matrix $W$ is called a {\color{blue} random matrix} if the entires $W_{ij}$ are random variables.
\end{definition}
...
Then there exists a unique measure $\mu \times \nu$, called the {\color{blue} product measure}, on $(\calX \times \calY, \calA \lor \calB)$ such that 
$$ (\mu \times \nu)(A \times B) = \mu(A) \nu(B), $$
...
Two random vectors, $X \in \bbR^n$ and $Y \in \bbR^m$ are {\color{blue} independent} if 
